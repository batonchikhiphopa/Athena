...
const PURPOSES = {
  ...
  "client/src/lib/api.ts": "Низкоуровневый HTTP-клиент backend API с классификацией ошибок.",
...
const FUNCTION_PURPOSES = {
  "client/src/App.tsx#App": "Собирает внешний access flow приложения: выбор vault-профиля, локальную разблокировку и переход в AthenaWorkspace.",
  ...
  "server/app.ts#createApp": "Строит Express application: parsing, открытые локальные API routes, error handling и static client.",
  "server/db/canonical-schema.ts#ensureCanonicalSchema": "Создаёт каноническую схему в пустой базе, удаляет auth-таблицы точной предыдущей схемы и отклоняет остальные fingerprint.",
  "server/db/canonical-schema.ts#runDatabaseIntegrityChecks": "Запускает явные SQLite quick check и foreign-key check.",
...
const DIRECTORY_PURPOSES = {
  ...
  "server/db/": "SQLite bootstrap и жизненный цикл канонической схемы",
...
function sectionFor(file) {
  ...
  if (file === "schema.sql") return "Database schema";
